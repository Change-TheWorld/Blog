...
router.get("/authors/:author_id",function(req,res){
    ...
    // });
        ArticleModel.find({author:req.params.author_id}).populate(["author"]).sort("-createDate").exec(function(err,docs){
            if(docs==null){
                res.render("404page",{errorMsg:"不存在该用户"});
                return;
            }
            var selfFlag="";
            var user="";
            ...
            var title="博客 - "+docs[0].author.username;
            var author=docs[0].author;
            res.render("author",{author:author,user:user,article:doc,title:title,dateSortArticles:dateSortArticles,commentSortArticles:commentSortArticles,visitedSortArticle:visitedSortArticle,praiseSortArticle:praiseSortArticle,tags:tags});
          });
});
...
router.get("/setting",function(req,res){
    ...
    res.render("building",{user:user,title:"设置模块正在建设中"});
});
...
router.get("/personalMsg",function(req,res){
    ...
    res.render("building",{user:user,title:"私信模块正在建设中"});
});
