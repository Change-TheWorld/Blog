		    	...
		    	success:function(data,resText,jqXHR){
                    ...
                    if(data.authors.length>0){
                          data.authors.forEach(function(author){
                             $(".top-nav-center-search-result-authors").append("<li><a href='/authors/"+author._id+"' >"+author.username+"</a>职位("+author.job+")</li>");
                          });
                    }
                    if(data.articles.length>0){
                    	data.articles.forEach(function(article){
                             $(".top-nav-center-search-result-articles").append("<li><a href='/articles/"+article._id+"' >"+article.title+"</a>点赞数量("+article.praiseNum+")</li>");
                    	});
                    }
		    	}
		    });
